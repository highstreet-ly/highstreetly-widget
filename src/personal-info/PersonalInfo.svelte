<script>
  ...
  let submitPayment = async () => {
    ...
    await draftOrderApi.assignRegistrant((innerdo) => {
      return innerdo.ownerEmail != null
    })
            ...
            <div class="sonaticket-flow col-span-full md:col-span-2">
              {#if wasPartiallyFulfilled}
                <div class="bg-red-100 py-3 px-5 mb-10">
                  <h5 class="font-bold">
                    The following items are currently unavailable. Your order
                    ...
                    {#if evt.isToTable}
                      <label class="checkbox-inline mr-3">
                        <input
                          class="mr-1"
                          type="checkbox"
                          bind:checked={draftOrder.isToTable}
                          value={1}
                          autocomplete="off"
                          on:click={handleToTableClick}
                        />
                        Deliver to table
                      </label>
                    {/if}
              ...
              {#if draftOrder.isToTable}
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="form-group">
                    <label for="payment-name">Table #</label>
                    <input
                      class="form-control"
                      id="table-number"
                      type="text"
                      ...
                      required
                      autocomplete="off"
                      bind:value={draftOrder.tableInfo}
                    />
                  </div>
                </div>
              {/if}
              ...
              <div style="background-color:#F2F6F6">
                <div class="p-4">
                  ...
                  <div class="text-right mt-3">
                    <small
                      >Subtotal: &pound{(cart.subtotal / 100).toFixed(2)}</small
                    >
                  </div>
