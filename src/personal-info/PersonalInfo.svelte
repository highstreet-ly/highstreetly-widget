<script>
    ...
    let s;
    export let draftOrder;

    export let pricedOrder;
    ...
    onMount(async () => {
        ...
        if (draftOrder.then){
            draftOrder = await draftOrder
        }

        if (pricedOrder.then){
            let list = await pricedOrder
            pricedOrder = list[0]
        }
                        ...
                        {#if wasPartiallyFulfilled}
                            <div class="bg-red-100 py-3 px-5 mb-10">
                                <h5 class="font-bold">
                                    ...
                                    unavailable. Your order has been
                                    ...
                                    {#each wasPartiallyFulfilledArray as unfulfilled}
                                        <li class="text-lg">
                                            ...
                                            {#if unfulfilled.reservedTickets > 0}
                                                only {unfulfilled.reservedTickets}
                                                available
                                            {:else}
                                                out of stock
                                            {/if}
                            ...
                            <div class="form-group">
                                ...
                                {#if postcodeInvalid}
                                    <div
                                        ...
                                        class="mt-2">
                                        <span class="message" style="color:red"
                                            >Your postcode isn't currently
                                            supported by this shop</span>
                                    </div>
                                {/if}
                        ...
                        <div style="background-color:#F2F6F6">
                            <div class="p-4">
                                ...
                                <div class="text-right">
                                    <small
                                        >Service Fee: &pound{cart.fee.toFixed(
                                            2
                                        )}</small>
                                </div>
                                <div class="text-right">
                                    <small
                                        >Delivery Fee: &pound{cart.deliveryFee.toFixed(
                                            2
                                        )}</small>
                                </div>
                                <div class="text-right mt-3">
